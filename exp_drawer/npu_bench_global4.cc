// npu_bench_global4.cc — NPU benchmark using Global4-core mode.
//
// Global4 mode: 4 Local Cores within one Cluster work together to process a
// single input, reducing latency for large models. Requires a model compiled
// with inference_scheme="global4".
//
// Build:
//   make -f Makefile.npu npu_bench_global4
//
// Usage:
//   ./npu_bench_global4 <model_global4.mxq> <num_runs> <model_gops> <model_gbytes>
//
// Output (stdout):
//   avg_latency_ms  <ms>
//   min_latency_ms  <ms>
...
static void usage(const char* prog) {
    fprintf(stderr, "Usage: %s <model_global4.mxq> <num_runs> <model_gops> <model_gbytes>\n", prog);
    fprintf(stderr, "  model compiled with inference_scheme=\"global4\" is required.\n");
}
...
int main(int argc, char* argv[]) {
    ...
    // --- Configure Global4-core mode (use Cluster1) ---
    mobilint::ModelConfig mc;
    if (!mc.setGlobal4CoreMode({
            mobilint::Cluster::Cluster1,
        })) {
        fprintf(stderr, "Failed to set Global4-core mode.\n");
        return 1;
    }

    // --- Load model (must be compiled with inference_scheme="global4") ---
    ...
    // --- Prepare dummy input (Global4 takes 1 input, split across 4 cores) ---
